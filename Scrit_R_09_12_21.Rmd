...
```{r}
liste_rht<-read.csv("liste_genotypes.csv")
liste_rht<-liste_rht %>% 
...
```
# Création de la variable RHT_voisin
```{r}
...
```
# Retrait des couples_pots pour lesquel Biomasse_seche n'existe pas
```{r}
...
```
# Retrait des couples_pots pour lesquel Biomasse_seche <24
```{r}
...
# Ajustement des types de toutes les données
```{r}
data_final<-data_final %>% 
  mutate(plante_genotype=as.factor(plante_genotype),
         RHT_rht=as.factor(RHT_rht),
         Nombre_de_feuille_ligulee=as.numeric(Nombre_de_feuille_ligulee),
