...
data_final2<-as_data_frame(read.csv("Mesures_neighbors_final_2.csv",header=TRUE,sep="\t"))
setdiff(data_final,data_final2)
names(data_final)
...
data_final$Condition_hydrique <-as.factor(data_final$Condition_hydrique)
data_final$Monoculture_Couple <-as.factor(data_final$Monoculture_Couple)
data_final$voisin <-as.factor(data_final$voisin)
data_final$Ligne<-as.factor(data_final$Ligne)
data_final$Colonne<-as.factor(data_final$Colonne)
```
...
data$var <- data$poids_graine
moy_longueur_feuille_2<-mean(data$Longueur_feuille_2)
sd_longueur_feuille_2<-sd(data$Longueur_feuille_2)
boxplot((data$var-moy_longueur_feuille_2)/sd_longueur_feuille_2~data$plante_genotype)
boxplot((data$var-moy_longueur_feuille_2)/sd_longueur_feuille_2~data$voisin)
...
modjd2 = lm (Longueur_feuille_2~
               +plante_genotype
              + voisin
             ,data=data[data$plante_genotype%in%c("235","114","329","376","395"),])
anova(modjd2)

modjd3 = lm (Longueur_feuille_2~
               +plante_genotype
              + voisin
             ,data=data[data$plante_genotype%in%c("412","165","131","74","130"),])
anova(modjd3)

modjd4 = lm (Longueur_feuille_2~
               +plante_genotype
              + voisin
             ,data=data[data$plante_genotype%in%c("482","348","265","89","118"),])

anova(modjd4)

mat_mod<-model.matrix(modjd1)[,-c(1)]
mat_cor<-cor(mat_mod)
mat_cor_t<-cor(t(mat_mod))
names(which(apply(t(mat_mod),FUN=sd,MARGIN=2)==0))
view(data_final[names(which(apply(t(mat_mod),FUN=sd,MARGIN=2)==0)),])
corrplot(cor(table(data_final$plante_genotype,data_final$voisin)))
plot(hclust(as.dist(1-cor(table(data_final$plante_genotype,data_final$voisin)))))
...
marginal_dge = emmeans(modjd0, ~ plante_genotype)
...
modDGE <- mmer(Longueur_feuille_2 ~ Structuration_design+ Bloc_M + poids_graine,
...
```{r}
modDGE_v <- mmer(Longueur_feuille_2 ~ Structuration_design +Bloc_M + poids_graine,
